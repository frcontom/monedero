...
export function GoalDetailView({ goalId }: { goalId: string }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Link href="/metas" className="text-sm text-blue-600 hover:underline">
        ← Metas
      </Link>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            {goal.description && <p className="mt-2 text-sm text-slate-600">{goal.description}</p>}
          </div>
          <button
        ...
        {!isDone && (
          <div className="mt-4 flex flex-wrap gap-2">
            {isActive && (
              <>
                <button onClick={() => runStatus("PAUSED")} className="rounded-lg bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-200">
                  Pausar
                </button>
      ...
      {p && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <div>
              <p className="text-xs text-slate-500">Necesario / día</p>
              <p className="font-semibold">{formatMoney(Math.round(p.requiredDaily))}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Necesario / semana</p>
              <p className="font-semibold">{formatMoney(Math.round(p.requiredWeekly))}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Necesario / mes</p>
              <p className="font-semibold">{formatMoney(Math.round(p.requiredMonthly))}</p>
            </div>
            <div>
              <p className="text-xs text-slate-500">Fecha proyectada</p>
